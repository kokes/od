...
def parse_listing(et, ico):
    vyp = et.find('./are:Odpoved/D:Vypis_OR', namespaces=et.nsmap)
    if vyp is None:
        print(f'{ico} nacteno, ale neobsahuje vypis dat')
        return None
    ...
    ico = int(ico)  #  lepsi nez - int(udaje['ico']) - da se pak dohledat

    ret['udaje'].append({'ico': ico, **udaje})
    ...
    for pfobj in vyp.findall('./D:ZAU/D:PFO', namespaces=et.nsmap):
        ...
        ret['pravni_formy'].append({'ico': ico, **pfo})
