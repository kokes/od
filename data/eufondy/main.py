...
def prehled_2017_2013(outdir: str, partial: bool = False):
    ...
    source_url = 'http://dotaceeu.cz/Dotace/media/SF/Informace%20o%20%c4%8derp%c3%a1n%c3%ad/Seznamy%20p%c5%99%c3%adjemc%c5%af%20(List%20of%20Beneficiaries)/2016/Seznam-prijemcu-05_2017.xlsx'
    with TemporaryDirectory() as tmpdir:
        target_filename = os.path.join(tmpdir, "workbook.xlsx")
        urlretrieve(source_url, target_filename)
    ...
    with open(os.path.join(outdir, 'prehled_2017_2013.csv'), 'w', encoding='utf8') as fw:
        cw = csv.writer(fw)
